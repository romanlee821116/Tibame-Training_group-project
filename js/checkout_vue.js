...
$(function () {
  // 按數量增加變色
  $('.checkout_list').on('mousedown', '.checkout_add', function () {
    $(this).css('background-color', '#172852');
    $(this).css('color', '#ffffff');
  });

  $('.checkout_list').on('mouseup', '.checkout_add', function () {
  ...
  // 按數量減少變色
  $('.checkout_list').on('mousedown', '.checkout_less', function () {
  ...
  $('.checkout_list').on('mouseup', '.checkout_less', function () {
    let val = $(this).next().val();
    if (val > 1) {
      $(this).css('background-color', '#ffffff');
      $(this).css('color', '#172852');
  ...
  // 若數量大於1，減號變白底
  $('.checkout_list').on('click', '.checkout_add', function () {
  ...
  // 數量等於1，減號變灰底
  $('.checkout_list').on('click', '.checkout_less', function () {
    let val = $(this).next().val();
    if (val == 1) {
  ...
  // 偵測input手動改數字，改變減號底色
  $('.checkout_list').on("keypress keyup blur", '.checkout_quantity' ,function (e) {
    // 不能輸入符號
