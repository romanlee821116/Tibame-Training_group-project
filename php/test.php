...
<form id="idFormAioCheckOut" method="post" action="Ecpay_CreateOrder.php">
    ...
    <!-- <label>動作 (Action):<input type="text" name="ServiceURL" class="form-control"/></label> -->
    ...
    <label class="col-xs-12">金額 (TotalAmount):
        <input type="text" name="TotalAmount" value="336" class="form-control"/>
        請帶整數，不可有小數點 僅限新台幣 金額不可為 0 元 CVS&BARCODE 最低限制為 30 元，最高限制為 30,000 元
    </label>
    <label class="col-xs-12">描述 (TradeDesc):
        <input type="text" name="TradeDesc" value="商品費用" class="form-control"/>
    </label>
    <label class="col-xs-12">名稱 (ItemName):
        <input type="text" name="ItemName" value="讚啦#aaa" class="form-control"/>
        商品名稱以符號 # 分
    </label>
